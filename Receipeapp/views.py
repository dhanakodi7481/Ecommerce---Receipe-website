...
class CategoryView(View):
    def get(self,request,val):
        ...
        if request.user.is_authenticated:
            totalitem = len(Cart.objects.filter(user=request.user))
        catview = Products.objects.filter(category=val)
...
class CategoryTitle(View):
    def get(self,request,val):
        catview = Products.objects.filter(title=val)
        title = Products.objects.filter(category=catview[0].category).values('title')
        totalitem = 0
        if request.user.is_authenticated:
            totalitem = len(Cart.objects.filter(user=request.user))
        return render(request,"app/category.html",locals())
...
class ProductDetail(View):
    def get(self,request,pk):
        catview = Products.objects.get(pk=pk)
...
class checkout(View):
    def get(self,request):
        ...
        totalamount = sum(item.product.discounted_price * item.quantity for item in cart_items) + 40
...
class Productdetail(View):
    def get(self, request, pk):
        catview = get_object_or_404(Products, pk=pk)
        return render(request, "app/productdetail.html", {"catview": catview})
